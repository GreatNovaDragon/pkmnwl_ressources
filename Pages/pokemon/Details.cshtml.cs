...
public class DetailsModel : PageModel
{
    ...
    public List<(Type, int)> GetWeaknesses()
    {
        ...
        var type_1 = _context.DamageRelations.Include(damageRelations => damageRelations.noDamageFrom)
            .FirstOrDefault(e => e.Type == Pokemon.Type1);
        if (Pokemon.Type2 == null)
        {
            foreach (var v in type_1.doubleDamageFrom) returnlist.Add((v, 200));

            foreach (var v in type_1.noDamageFrom) returnlist.Add((v, 0));

            foreach (var v in type_1.halfDamageFrom) returnlist.Add((v, 50));
        }
        else
        {
            ...
            {
                var mult = 100;
                if (type_1.noDamageFrom.Contains(t)) mult *= 0;
                if (type_2.noDamageFrom.Contains(t)) mult *= 0;
                if (type_1.halfDamageFrom.Contains(t)) mult /= 2;
                if (type_2.halfDamageFrom.Contains(t)) mult /= 2;
                if (type_1.doubleDamageFrom.Contains(t)) mult *= 2;
                if (type_2.doubleDamageFrom.Contains(t)) mult *= 2;
